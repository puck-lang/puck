...
function getSubPatterns(pattern) {
  ...
  if ($unwrapTraitObject(__PUCK__value__45).kind == "RecordType") {
    ...
    var __PUCK__value__47 = _core.Iterable['$List<E>'].map.call({ type: '$List<E>', value: record.properties, $isTraitObject: true }, function (p) {
      return p.pattern;
    });
...
function checkExhaustiveEnum(patterns, type_, enum_, scope) {
  ...
  _core.ObjectMap.forEach.call(enumArmsMap, function (_ref) {
    ...
    var __PUCK__value__75 = _core.Iterable['$List<E>'].enumerate.call({ type: '$List<E>', value: getSubPatterns($unwrapTraitObject(patterns)[0]), $isTraitObject: true });
